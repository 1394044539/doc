...
- 3、幻读：系统管理员A将数据库中所有学生的成绩从具体分数改为ABCDE等级，但是系统管理员B就在这个时候插入了一条具体分数的记录，当系统管理员A改结束后发现还有一条记录没有改过来，就好像发生了幻觉一样，这就叫幻读。
  ...
  幻读可以更具体理解：需要先校验重复，再插入，用select并没有查询出重复数据，但是insert却报错唯一索引重复
...
1. 读未提交
   ...
   事务A在执行过程中，事务B更新了数据，但还未提交，此时事务A已经读取到了B更新后的数据，此时如果B被回滚了，A读到的就是错误的数据，就引发了脏读问题

2. 读已提交

   事务A在执行过程中，事务B更新了数据，但还未提交，此时事务A读取到的任然是更新前的数据，解决了脏读的问题。但B后面提交了事务，A的事务再去读取，就能读到B更新的数据，此时A和上一次读取到的数据不一致，就引发了不可重复读的问题。

3. 可重复读（mysql的默认隔离级别就是这个）

   事务A在执行过程中，事务B新增了数据，并完成了提交，此时A多次查询，查询出的结果任然是B提交前的数据，这就解决了不可重复读的问题。如果此时A事务进行更新操作，则无法更新到B新增的数据，此时则发生了幻读问题。

   在该种隔离级别情况下，如果事务A和事务B同时操作同一行数据，如果事务A对数据减50，事务B对数据也减50，此时A在减完之后查询出来的结果则是被减去了100，因为该种隔离级别select是读取历史版本，insert、update和delete读取的是当前版本。

4. 串行化

   简单粗暴，在事务执行过程中会锁表，相当于单线程操作了，基本上不会使用到这种隔离级别。
...
（1）mysql常用的完成时间类型一般是两种，datetime和timestamp

- timestamp
  - 邮戳类型，保存年-月-日 时-分-秒
  - 会把传入的值转化为UTC（世界标准时间）进行存储，然后查询时再转换回去
  - 存储范围：’1970-01-01 00:00:01.000000’ 到 ‘2038-01-19 03:14:07.999999’
- datetime
  - 日期时间类型
  - 原样存储
  - 存储范围：’1000-01-01 00:00:00.000000’ 到 ‘9999-12-31 23:59:59.999999’

（2）计算时间差值：

mysql对时间类型会有一个隐式转换，比如2022:02:09 23:12:45这种时间，会直接被拼成字符串20220209231245，如果用这个时间减去另一个时间，比如2022:02:09 23:11:45，就会变成20220209231245-20220209231145=100，这种很明显不是我们需要的，所以需要用时间函数进行计算。

- timestampdiff

  ```sql
  timestampdiff(格式，被减数，减数)
  ```
...
- unix_timestamp
  ...
  ```mysql
  unix_timestamp(减数)-unix_timestamp(被减数)
  ```

  unix_timestamp其实是将时间转化为时间戳（秒级时间戳，如果需要转化为毫秒级，需要手动*1000）
